...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale ja-jp /uninstall /silent /vendor "Vendor Name" /productName "ClangNet" /mediaBookList "A Sandcastle Documented Class Library" > NUL
...
REM build time.
IF EXIST "ClangNet.msha" COPY /Y "ClangNet.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale ja-jp /wait 0 /operation uninstall /vendor "Vendor Name" /productName "ClangNet" /bookList "A Sandcastle Documented Class Library" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale ja-jp /wait 0 /operation install /sourceUri "%CD%\ClangNet.msha"
